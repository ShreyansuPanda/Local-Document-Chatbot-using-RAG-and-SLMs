...
with st.sidebar:
    ...
    temperature = st.slider(
        ...
        min_value=0.0,
        max_value=1.5,
        value=float(MODEL_CONFIGS[model]["temperature"]),
        step=0.05,
    ...
    if (st.session_state.qa_system.model_name != model or
        ...
        st.session_state.qa_system.chunk_overlap != chunk_overlap or
        getattr(st.session_state.qa_system, 'temperature', None) != temperature):
        try:
            st.session_state.qa_system = DocumentQASystem(
                model_name=model,
            ...
            # Update temperature in model config and LLM
            st.session_state.qa_system.model_config["temperature"] = temperature
            if hasattr(st.session_state.qa_system, "llm"):
                st.session_state.qa_system.llm.temperature = temperature
            ...
            try:
                ...
                for chat in st.session_state.chat_history:
                    with st.expander(f"Q: {chat['question']} ({chat['timestamp']}) - Model: {chat['model']}"):
                        if chat["question"] != chat.get("corrected_question", chat["question"]):
                            st.markdown(f"**Corrected Question:** {chat['corrected_question']}")
                        st.markdown("**Answer:**")
